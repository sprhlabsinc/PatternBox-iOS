...
#define BOARD_W     150
#define BOARD_H     80
#define kCustomIOS7AlertViewDefaultButtonHeight         40
...
- (void)show
{
    ...
    // Attached to the top most window (make sure we are using the right orientation):
        ...
        [self setFrame:CGRectMake(0, 0, self.parentView.frame.size.width, self.parentView.frame.size.height)];
...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, BOARD_W, BOARD_H)];
    }
    ...
    UIView *dialogContainer = [[UIView alloc] initWithFrame:CGRectMake((screenSize.width - dialogSize.width) / 2, (screenSize.height - 64 - dialogSize.height) * 0.5+44, dialogSize.width, dialogSize.height)];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if ([buttonTitles[i] isEqualToString:_okButtonTitle]) self.okButton = closeButton;
    }
}
...
- (CGSize)countScreenSize
{
    ...
    if (UIInterfaceOrientationIsLandscape(interfaceOrientation)) {
        ...
        screenWidth = (tmp > screenHeight)? tmp : screenHeight;
        screenHeight = (tmp < screenHeight)? tmp : screenHeight;
    }
...
- (void)keyboardWillShow: (NSNotification *)notification
{
    ...
    if (UIInterfaceOrientationIsLandscape(interfaceOrientation)) {
        ...
        keyboardSize.width = (tmp > keyboardSize.height)? tmp : keyboardSize.height;
        keyboardSize.height = (tmp < keyboardSize.height)? tmp : keyboardSize.height;
    }
